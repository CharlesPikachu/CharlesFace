...
# training explain refer to "region_loss.py" in loss_module.
	...
	def forward(self, output, target=None):
		...
		if self.training:
			...
			nGT, nCorrect, coord_mask, conf_mask, cls_mask, tx, ty, tw, th, tconf, tcls = \
				build_targets(pred_boxes, target.data, masked_anchors, nA, nC, nH, nW, self.noobject_scale, self.object_scale, self.thresh, self.seen)
			# nGT, nCorrect, coord_mask, conf_mask, cls_mask, tx, ty, tw, th, tconf,tcls = \
			# 	build_targets(pred_boxes, target.data, self.anchors, nA, nC, nH, nW, self.noobject_scale, self.object_scale, self.thresh, self.seen)
			cls_mask = (cls_mask == 1)
			...
			return loss
		# get pred boxes while test
		else:
			masked_anchors = []
			# Usage of achor_mask:
			# 	refer: https://github.com/pjreddie/darknet/issues/558
			# 	content(Author reply):
			# 		Every layer has to know about all of the anchor boxes but is only predicting some subset of them. 
			# 		This could probably be named something better but the mask tells the layer which of the bounding boxes it is responsible for predicting.
			# 		The first yolo layer predicts 6,7,8 because those are the largest boxes and it's at the coarsest scale.
			# 		The 2nd yolo layer predicts some smallers ones, etc.
			for m in self.anchor_mask:
				masked_anchors += self.anchors[m*self.anchor_step: (m+1)*self.anchor_step]
			# default stride = 32 because of image_width = 416 and output_width = 13
			# 416 // 13 = 32
			masked_anchors = [anchor/self.stride for anchor in masked_anchors]
